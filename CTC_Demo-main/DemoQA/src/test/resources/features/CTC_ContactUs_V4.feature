...
Feature: CTC-QA Automation Demo
  ...
  Scenario Outline: Fill the contact us form and submit
    ...
    Then Send Popup that "Here we select customer from the dropdown menu"
    And I select a value "<customer>" from dropdown field
    Then Send Popup that "Here we select the interested in checkbox"
    And I select "<interested_in>" checkbox
    Then Send Popup that "Here we select the contact radio button"
    And I select "<contact>" radio button
    Then Send Popup that "Here we enter the description"
    ...
    Then Send Popup that "Here we verify the message was sent successfully"
    Then Send Popup that "Test 2 was completed"
    And Quit the driver
